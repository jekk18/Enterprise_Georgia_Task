...
<div class="max-w-7xl mx-auto px-4 py-12">
    {{-- სათაურის სექცია --}}
    <div class="mb-12 border-gray-100 pb-8 flex justify-between items-end"> 
        <h1 class="text-4xl font-black text-gray-900 tracking-tight">{{ __('messages.all_articles') }}</h1>  
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($posts as $post)
            {{-- აქ ვიღებთ მიმდინარე თარგმანს --}}
            @php $translate = $post->translations->first(); @endphp
            ...
            <div class="group bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300 overflow-hidden flex flex-col">
                ...
                <div class="relative h-56 overflow-hidden">
                    {{-- სურათი ახლა მოდის თარგმანიდან ($translate) --}}
                    @if($translate && $translate->image)
                        <img src="{{ asset('storage/' . $translate->image) }}" 
                ...
                <div class="p-6 flex flex-col flex-grow">
                    ...
                    <div class="flex justify-between items-center mt-6">
                        <div class="flex items-center text-gray-400 gap-1.5">
                            ...
                            {{-- აქ დარწმუნდი რომ კონტროლერში გაქვს ->withCount('comments') --}}
