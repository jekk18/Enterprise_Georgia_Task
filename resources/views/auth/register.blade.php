<x-layout>
  <div class="h-screen bg-linear-to-r from-indigo-100 from-10% via-sky-100 via-30% to-emerald-100 to-90% flex justify-center items-center w-full px-4 overflow-y-auto py-10">
    ...
    <form action="{{ route('register', app()->getLocale()) }}" method="POST" class="w-full max-w-md">
        @csrf
        
        <div class="bg-white px-8 py-10 rounded-2xl shadow-2xl w-full">
            <div class="text-center mb-8">
                <h1 class="text-3xl font-bold text-gray-800">{{ __('messages.register_title') }}</h1>
                <p class="text-gray-500 text-sm mt-2">{{ __('messages.register_subtitle') }}</p>
            </div>
                ...
                <div>
                    <div class="flex items-center border-2 py-3 px-4 rounded-xl focus-within:border-blue-500 transition group">
                        ...
                        <input class="pl-3 outline-none border-none w-full text-gray-700 bg-transparent" 
                               ...
                               placeholder="{{ __('messages.name_placeholder') }}" required autofocus/>
                    </div>
                    @error('name')
            ...
            <div class="mt-6 text-center">
                <p class="text-sm text-gray-500">
                    ...
                    <a href="{{ route('login', app()->getLocale()) }}" class="text-blue-600 hover:underline font-bold">
